...
//if(0) // FIXME
  ...
  file << theTracker->idToDet(id)->subDetector();
  file << HitInfo::getInfo(recHit);
  file << " | ";
...
*/
  ...
  int nstrips = stripDet->specificTopology().nstrips();

  double lx = x * (recHit.cluster()->barycenter() / (nstrips/2) - 1);

  lpos = LocalPoint(lx,y,0);
